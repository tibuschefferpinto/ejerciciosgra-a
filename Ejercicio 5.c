...
bool modificarDato(int* pLista, int N, int M, int nuevoValor);
...
void loop() {
  ...
  // Ejemplo: modificar el dato en la posición M
  if (modificarDato(pLista, N, M, 999)) {
    Serial.print("Dato en la posicion ");
    Serial.print(M);
    Serial.println(" modificado a 999.");
  } else {
    Serial.println("Error al modificar: posicion fuera de rango.");
  }

  // Mostrar lista nuevamente para ver el cambio
  mostrarLista(pLista, N);

  // Pausa larga para no repetir infinitamente
  while (true);
}
//===========================
void cargarLista(int *pLista, int N) {
...
bool modificarDato(int* pLista, int N, int M, int nuevoValor) {
  if (M >= 0 && M < N) {
    pLista[M] = nuevoValor;// modifica el valor en la posición M
    return true;//éxito
  } else {
    return false;//error
  }
